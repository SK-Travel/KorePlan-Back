	...
	public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
		...
		if (optionalUser.isPresent()) {
			...
			String redirectUrl = UriComponentsBuilder.fromUriString("http://localhost:5173/sign-up")
					.queryParam("email", userInfo.getEmail()).queryParam("name", userInfo.getName()).build()
					.toUriString();
...
//            );
	...
	private UserEntity registerNewUser(OAuth2UserInfoEntity userInfo) {
		...
		UserEntity newUser = UserEntity.builder().loginId(randomLoginId).password(EncryptUtils.hashPassword("oauth")) // 기존에
																														// 만든
																														// 해시
																														// 유틸리티로
																														// 비밀번호
																														// 해싱
				.name(userInfo.getName()).email(userInfo.getEmail()).phoneNumber(phonePlaceHolder).build();
